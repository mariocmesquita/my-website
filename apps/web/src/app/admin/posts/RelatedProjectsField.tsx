...
export function RelatedProjectsField({ name, projects }: RelatedProjectsFieldProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-sm font-medium text-foreground">Projetos relacionados</label>
      ...
      <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            <span className="text-foreground/50">
              {selectedIds.length === 0
                ? 'Selecionar projetos...'
                : `${selectedIds.length} projeto${selectedIds.length > 1 ? 's' : ''} selecionado${selectedIds.length > 1 ? 's' : ''}`}
            </span>
        ...
        <PopoverContent className="w-[300px] p-0" align="start">
          <Command>
            <CommandInput placeholder="Buscar projetos..." />
            <CommandList>
              ...
              <CommandGroup>
                {projects.map((project) => (
                  ...
                  >
                    <Check
                      size={14}
                      className={selectedIds.includes(project.id) ? 'opacity-100' : 'opacity-0'}
                    />
                    {project.title}
                  </CommandItem>
                ))}

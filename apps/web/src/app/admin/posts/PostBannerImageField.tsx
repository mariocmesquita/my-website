...
export function PostBannerImageField({ name, label }: PostBannerImageFieldProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadedUrl = await uploadPostFile(token, file);
    ...
    <div className="flex flex-col gap-1">
      ...
      {url ? (
        <div className="relative w-full overflow-hidden rounded-lg border border-brand">
          ...
          <img src={url} alt="Banner" className="h-36 w-full object-cover" />
          <button
            type="button"
        ...
        <label
          ...
          className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-brand/40 p-6 transition hover:border-brand/70 hover:bg-brand/5 aria-disabled:cursor-not-allowed aria-disabled:opacity-60"
        >
          {isUploading ? (
